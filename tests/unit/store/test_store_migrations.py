import sqlite3
from importlib import import_module
from typing import Protocol, cast


class _ApplyMigrations(Protocol):
    def __call__(self, connection: sqlite3.Connection) -> None: ...


def _apply_migrations(connection: sqlite3.Connection) -> None:
    module = import_module("locontext.store.migration_runner")
    apply_migrations = cast(
        _ApplyMigrations | None, getattr(module, "apply_migrations", None)
    )
    if apply_migrations is None:
        raise AssertionError(
            "expected locontext.store.migration_runner.apply_migrations"
        )
    apply_migrations(connection)


def test_apply_migrations_bootstraps_schema_and_records_version(
    connection: sqlite3.Connection,
) -> None:
    _apply_migrations(connection)
    table_rows = cast(
        list[tuple[str]],
        connection.execute(
            "SELECT name FROM sqlite_master WHERE type = 'table'"
        ).fetchall(),
    )
    table_names = {row[0] for row in table_rows}
    assert {
        "schema_migrations",
        "sources",
        "snapshots",
        "documents",
        "chunks",
        "chunk_fts",
        "chunk_fts_data",
        "chunk_fts_idx",
        "chunk_fts_docsize",
        "chunk_fts_config",
        "source_sets",
        "source_set_members",
    }.issubset(table_names)
    applied_versions = connection.execute(
        "SELECT version FROM schema_migrations ORDER BY version ASC"
    ).fetchall()
    assert applied_versions == [("0001_initial.sql",), ("0002_source_sets.sql",)]


def test_apply_migrations_is_a_no_op_when_all_versions_are_already_applied(
    connection: sqlite3.Connection,
) -> None:
    _apply_migrations(connection)
    before_versions = connection.execute(
        "SELECT version, applied_at FROM schema_migrations ORDER BY version ASC"
    ).fetchall()
    before_table_names = connection.execute(
        "SELECT name FROM sqlite_master WHERE type = 'table' ORDER BY name ASC"
    ).fetchall()
    _apply_migrations(connection)
    after_versions = connection.execute(
        "SELECT version, applied_at FROM schema_migrations ORDER BY version ASC"
    ).fetchall()
    after_table_names = connection.execute(
        "SELECT name FROM sqlite_master WHERE type = 'table' ORDER BY name ASC"
    ).fetchall()
    assert after_versions == before_versions
    assert after_table_names == before_table_names
